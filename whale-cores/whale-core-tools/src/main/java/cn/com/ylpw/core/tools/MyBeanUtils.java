...
//              }
  ...
  /****
	 * 
	 * <p>验证实体类中给定字段是否为空,字段中一个为空(null、"")返回False</p>
	 * @author L.T 2014-9-28 下午12:42:03
	 * @param entity  对象实例
	 * @param fields  字段名
	 * @return
	 */
	public static   boolean   fieldsIsNull (Object entity , String... fields){
		if(null==fields||fields.length<1)return true;
		for (String field : fields){
			...
			try {
				...
				// 得到字段名称
				Field thefield = entity.getClass().getDeclaredField(field);
				
				// 得到设值方法
				Method method = entity.getClass().getDeclaredMethod(methodName);
						
				// 得到字段类型
				String parameterType = thefield.getType().getSimpleName();
				if (parameterType.equals("String")){
					if (null==method.invoke(entity)||StringUtils.isBlank(method.invoke(entity).toString())){
						return true  ;
					}
				}			
			} catch (Exception e) {
