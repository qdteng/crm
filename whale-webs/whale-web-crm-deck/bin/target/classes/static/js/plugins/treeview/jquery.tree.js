    ...
    $.fn.treeview = function(settings) {
        var dfop =
            {
                ...
                url: false, //异步请求的url
                cbiconpath: "/css/plugins/treeview/images/icons/", //checkbox icon的目录位置
                icons: ["checkbox_0.gif", "checkbox_1.gif", "checkbox_2.gif"],
				emptyiconpath: "/css/plugins/treeview/images/s.gif", //checkbxo三态的图片
        ...
        function getck(items, c, fn) {
            for (var i = 0, l = items.length; i < l; i++) {
            	//注释原取选中菜单值方法,获取三级菜单的父级菜单ID.并且菜单刚开始的 全部 节点不保存   T.L 2012-10-24
                //(items[i].showcheck == true && items[i].checkstate == 1) && c.push(fn(items[i]));
                //(items[i].showcheck == true && items[i].checkstate != 0 && fn(items[i]) != 2) && c.push(fn(items[i]));//ZHaoHb2017.03.13
            	(items[i].showcheck == true && items[i].checkstate != 0 && fn(items[i]) != -1) && c.push(fn(items[i]));
